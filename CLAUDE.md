...
## Configurable review (multi-type checklists + UI-editable prompt) — implemented

- **Item types** — checklist items carry a `type`: `boolean` (`{value,confidence}`), `category` (one of `options`, optional `compliant_values`), `multi_boolean` (`options:[{key,label}]` → `{values:{sub:bool},confidence}`), `text` (`{text,confidence}`, informational/never graded), `number` (optional `min`/`max`, optional `compliant_range`). Defined + validated in `evas/checklists.py` (`ITEM_TYPES`, `validate_items`); normalized tolerantly in `evas/ai.py` (`_normalize_findings`); grading (`compute_video_grade`) sums per-item compliance — only boolean/multi_boolean always grade, category/number grade only when they declare a compliant target, text never.
- **Prompt = framing only** — `checklists.prompt_template` holds the UI-editable role/instructions prose; the AI layer always appends a fixed, type-aware output **contract** (per-item answer shapes + JSON spec) so findings stay parseable regardless of author input. NULL → `DEFAULT_FRAMING`. The A/B `prompt_version` path still loads versioned `prompts/*.txt` files whole; per-checklist `prompt_template` is used otherwise (`get_reviewer(prompt_version, prompt_template)`).
- **Checklist API + UI** — `evas/api/checklists_routes.py`: `GET/POST /clients/{id}/checklists` + `/checklists/active` (admin). Saving creates a new **version** (immutable; deactivates the prior active one for that name) — matches `UNIQUE(client_id,name,version)`. Frontend: a "Review config" editor in the Clients screen (per-type item editor + prompt textarea + grading mode), wired to the API.
- **Schema addition** — migration `0004` adds nullable `checklists.prompt_template` (mirrored into `evas_schema.sql`). `items`/`findings` stay `jsonb`, so the multi-type finding shapes need no DDL.
