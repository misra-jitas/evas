"""initial schema — reproduces evas_schema.sql verbatim

Revision ID: 0001
Revises:
Create Date: 2026-06-06

This migration intentionally embeds the exact DDL from evas_schema.sql (the
locked source of truth) rather than autogenerated operations, so the migrated
schema cannot drift from the canonical file.
"""
...
DROP TABLE IF EXISTS human_frame_notes;
DROP TABLE IF EXISTS human_reviews;
DROP TABLE IF EXISTS ai_frame_findings;
...
DROP TYPE IF EXISTS grading_mode;
DROP TYPE IF EXISTS video_priority;
...
def upgrade() -> None:
    # exec_driver_sql sends the DDL straight to psycopg2 without SQLAlchemy
    # bind-parameter parsing, so colons in comments (e.g. "HH:MM:SS") are safe.
